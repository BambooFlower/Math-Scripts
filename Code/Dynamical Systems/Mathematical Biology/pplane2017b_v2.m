%   pplane2017b_v2  is an interactive tool for studying planar autonomous systems of
%	differential equations.  When pplane2017b_v2 is executed, a pplane2017b_v2 Setup
...
%
%	When the Proceed button is pressed on the Setup window, the pplane2017b_v2
...
%	the pplane2017b_v2 Display window, the solution to the system with that
...
function output = pplane2017b_v2(action,input1,input2,input3)

    startstr = 'pplane2017b_v2';
    if nargin < 1
        action ='initialize';
    end
    ...
    if strcmp(action,'initialize')
        ...
        % First we make sure that there is no other copy of pplane2017b_v2
        % running, since this causes problems.
        pph = findobj('tag','pplane2017b_v2');
        if ~isempty(pph)
            qstring = {'There are some pplane2017b_v2 figures open although they may be invisible.  ';...
                'What do you want to do?'};
            tstring = 'Only one copy of pplane2017b_v2 can be open at one time.';
            b1str = 'Restart pplane2017b_v2.';
            b2str = 'Just close those figures.';
            b3str = 'Do nothing.';
            answer = questdlg(qstring,tstring,b1str,b2str,b3str,b1str);
            if strcmp(answer,b1str)
                delete(pph);
                %pplane2017b_v2;return
        ...
        % Next we look for old files created by pplane2017b_v2.
        ...
        for k = 1:length(oldfiles)
            ...
            ll = fgetl(fid);
            fclose(fid);
            if strcmp(ll,'%% Created by pplane2017b_v2')
                delete(fn)
            else
                l= length(ofiles);
                ofiles{l+1} = fn;
        ...
        switch style
            ...
            case 'white'
                color.temp = [1 0 0]; % red for temporary orbits
                color.orb = 'k';  % black for orbits
                color.eqpt = 'g';  % green for eq. pts.
                color.arrows = 0*[1 1 1]; % gray for arrows
                color.narrows = 0*[1 1 1];  % gray for nullcline arrows
                color.tx = [0 0 1]; % blue for xt plots & 3D plots
                color.ty = [1 0 0]; % red for yt plots
                color.ta = [1 0 0]; % red for axis plots
                color.sep = [0,1,0];% green for separatrices
                color.xcline = 'b'; % blue for xclines
                color.ycline = 'r'; %  red for yclines
        ...
        ud.color = color;
        ud.level = ' ';
        ppset = figure('name','pplane2017b_v2 Setup',...
            'tag','pplane2017b_v2','visible','off',...
            'user',ud);

        pplane2017b_v2('figdefault',ppset);
        ...
        butt(2) = uicontrol('style','push',...
            'pos',rwind,...
            'string','Revert',...
            'CallBack','pplane2017b_v2(''revert'')',...
            'visible','off');

        butt(3) = uicontrol('style','push',...
            'pos',pwind,...
            'string','Proceed',...
            'CallBack','pplane2017b_v2(''proceed'')',...
            'visible','off');
        ...
        callnfpts = [
            'ppset = findobj(''name'',''pplane2017b_v2 Setup'');',...
            'ud = get(ppset,''user'');'...
            'me = ud.h.npts;',...
        ...
        meload = uimenu(mefile,'label','Load a system ...',...
            'CallBack','pplane2017b_v2(''loadsyst'',''system'');',...
            'pos',1);
        mesave = uimenu(mefile,'label','Save the current system ...',...
            'CallBack','pplane2017b_v2(''savesyst'',''system'');',...
            'pos',2);
        meloadg = uimenu(mefile,'label','Load a gallery ...',...
            'CallBack','pplane2017b_v2(''loadsyst'',''gallery'');',...
            'separator','on','pos',3);
        mesaveg = uimenu(mefile,'label','Save a gallery ...',...
            'CallBack','pplane2017b_v2(''savesyst'',''gallery'');',...
            'tag','savegal','pos',4);
        ...
        merestart = uimenu(mefile,'label',...
            'Restart pplane2017b_v2',...
            'CallBack','pplane2017b_v2(''restart'')',...
            'separator','on','pos',12);

        mequit = uimenu(mefile,...
            'label','Quit pplane2017b_v2',...
            'CallBack','pplane2017b_v2(''quit'')',...
    ...
    elseif strcmp(action,'savesyst')
        ...
        switch type
            case 'system'
                ...
                fn = newsyst.name;
                if ~isempty(fn)
                    fn(abs(fn)==32)='_';   % Replace spaces by underlines.
                end
                fn = [fn, '.pps'];
                comp = computer;
                switch  comp
    ...
    elseif strcmp(action,'loadsyst')  % This loads either a system or a gallery.
        ...
        for j = 1:nnn
            ...
            sname = newsyst.name;
            sname(abs(sname) == 95) = ' '; % Replace underscores with spaces.
            newsyst.name = sname;
            ignore = pplane2017b_v2('addgall',newsyst);
            if ignore == -1
        ...
        if strcmp(type,'system') % Added a system.
            ...
            pplane2017b_v2('system',kk);
        end
        if strcmp('type','default')
            pplane2017b_v2('system',1);
        end
        set(sud.h.gallery,'enable','on');
        x = 1;
        xp = [xp(2),x,x,xp(2)];
    ...
    elseif strcmp(action,'proceed')
        ...
        sud.o = sud.c;
        set(ppset,'user',sud);
        he = findobj('name','pplane2017b_v2 Equilibrium point data');
        hl = findobj('name','pplane2017b_v2 Linearization');
        close([he;hl]);
        % set([he;hl],'vis','off');
        ...
        if (sud.flag == 1)
            ...
            NumbFPts = sud.c.npts;
            ppdisp = findobj('name','pplane2017b_v2 Display');
            dud = get(ppdisp,'user');
            ...
            if ( (NumbFPts ~= dud.syst.npts) || (any(WINvect ~= dud.syst.wind) ) )
                ...
                set(ppdisp,'user',dud);
                pplane2017b_v2('dirfield',ppdisp);
            end
            figure(ppdisp);
        else
            sud.flag = 1;
            ...
            txname = Xname;
            tyname = Yname;
            xderivstr(abs(xderivstr)==92)=[];
            yderivstr(abs(yderivstr)==92)=[];
            Xname(abs(Xname)==92)=[];
            Yname(abs(Yname)==92)=[];
                    ...
                    if isempty(pv)
                        ...
                        if isempty(str2num(pv)) % This is an expression.
                            ...
                            if (err || isempty(pval))
                                ...
                                fprintf('\a')
                                errordlg(errmsg,'pplane2017b_v2 error','on');
                                warning(SS)
                                return
                            end
                        end
                        xxderivstr = pplane2017b_v2('paraeval',pn,pv,xderivstr);
                        yyderivstr = pplane2017b_v2('paraeval',pn,pv,yderivstr);
            ...
            % If an old function m-file exists delete it, and then build a new
            % one.
            ...
            commstr = '%%%% Created by pplane2017b_v2\n\n';
            ...
            % Find pplane2017b_v2 Display if it exists.
            % If pplane2017b_v2 Display exists, update it.  If it does not build it.
            ...
            if (~isempty(ppdisp))
                ...
                ppdisp = figure('name','pplane2017b_v2 Display',...
                    ...
                    'interrupt','on',...
                    'visible','off',...
                    'tag','pplane2017b_v2');
                pplane2017b_v2('figdefault',ppdisp);
                dud = get(ppdisp,'user');
                dud.syst = sud.c;
                ...
                dbutt(2) = uicontrol('style','push',...
                    ...
                    'string','Quit',...
                    'CallBack','pplane2017b_v2(''quit'')','visible','off');

                dbutt(3) = uicontrol('style','push',...
                    'pos',[buttl,ppaxb+buttsep,buttw,butth],...
                    'string','Print',...
                    'CallBack','pplane2017b_v2(''print'')','visible','off');
                ...
                merestart = uimenu(fmenu,'label',...
                    ...
                    'CallBack','pplane2017b_v2(''restart'')',...
                    'separator','on');
                mequit = uimenu(fmenu,'label','Quit pplane2017b_v2',...
                    'CallBack','pplane2017b_v2(''quit'')','separator','off');
                ...
                menu(3) = uimenu(megraph,'label',[txname,' vs. t'],...
                    'CallBack','pplane2017b_v2(''plotxy'',1)');

                menu(5) = uimenu(megraph,'label',[tyname,' vs. t'],...
                    'CallBack','pplane2017b_v2(''plotxy'',2)');

                meplot3 = uimenu(megraph,'label','Both',...
                    'CallBack','pplane2017b_v2(''plotxy'',3)');

                meplot4 = uimenu(megraph,'label','3 D',...
                    'CallBack','pplane2017b_v2(''plotxy'',4)');

                meplot5 = uimenu(megraph,'label','Composite',...
                    'CallBack','pplane2017b_v2(''plotxy'',5)');
                ...
                if dud.egg
                    ...
                    metest = uimenu(solmenu,'label','Test case',...
                        'CallBack','pplane2017b_v2(''test case'')',...
                ...
                meset   = uimenu(menu(1),'label','Settings.',...
                    'CallBack','pplane2017b_v2(''settings'')');
                ...
                meexportdata = uimenu(menu(1),'label','Export solution data.',...
                    'CallBack','pplane2017b_v2(''export'')',...
                ...
                menu(6) = uimenu(menu(1),'label','Make the Display Window inactive.',...
                    'CallBack','pplane2017b_v2(''hotcold'')','separator','on');
                ...
                set(findobj(gcf,'label','&Figure Toolbar'),...
                    'CallBack','pplane2017b_v2(''showbar'')');

                set(0,'showhiddenhandles',hhsetup);

                set(gcf,'WindowButtonDownFcn','pplane2017b_v2(''down'')');
                set(ppdisp,'WindowButtonMotionFcn','pplane2017b_v2(''cdisp'')');
            ...
            % Fix up nullcline menu. If the direction field type is nullclines,
            % always display nullclines initially. For other field types, take the
            % existing value.
            menull = findobj('tag','null');
            if strcmp(dud.syst.fieldtype,'nullclines')
                set(menull,'enable','on','label','Hide nullclines.');
            else
                lab_null = get(menull, 'label');
                set(menull,'enable','on','label',lab_null);
            end
            ...
            ppkbd = findobj('name','pplane2017b_v2 Keyboard input','vis','on');
            if ~isempty(ppkbd),pplane2017b_v2('kbd'),end
            pplevel = findobj('name','pplane2017b_v2 Level sets');
    ...
    elseif strcmp(action,'linear')
        ...
        % Get the information from pplane2017b_v2 Equilibrium ...
        ...
        % Find pplane2017b_v2 Linearization if it exists.
        % If pplane2017b_v2 Linearization exists, update it.  If it does not build it.

        pplin= findobj('name','pplane2017b_v2 Linearization');
        if (~isempty(pplin))
            figure(pplin);
            dud = get(pplin,'user');
            dud.syst = system;
            ...
            pplin = figure('name','pplane2017b_v2 Linearization',...
                ...
                'interrupt','on',...
                'visible','off',...
                'tag','pplane2017b_v2');
            pplane2017b_v2('figdefault',pplin);
            dud = get(pplin,'user');
            dud.dir = 0;
            dud.syst = system;
            dud.settings = settings;
            ...
            mequit = uimenu(fmenu,'label','Quit pplane2017b_v2',...
                'CallBack','pplane2017b_v2(''quit'')','separator','on','pos',9);
            ...
            set(pplin.Number,'WindowButtonDownFcn',['pplane2017b_v2(''down'',',num2str(pplin.Number),')']);
        ...
        if (~strcmp(dfcn,'') && exist(dfcn, 'file')==2), delete([tempdir,dfcn,'.m']);end
        tee = clock;
        tee = ceil(tee(6)*100);
        dfcn=['pptp',num2str(tee)];
    ...
    elseif strcmp(action,'dirfield')
        ...
        % Find pplane2017b_v2 Display and get the user data.
        ...
        N = dud.syst.npts;
        if strcmp(get(disph,'name'),'pplane2017b_v2 Linearization')
            N = floor(3*N/4);
        end
        ...
        if minx < 0 && 0 < maxx
            ...
            set(hx,'visible','off','color',color.xcline,'linestyle','--','LineWidth',2);
        else
            hx = zeros(0,1);
        end
        if miny < 0 && 0 < maxy
            [~,hy] = contour(Xx,Yy,DY,[0,0],'--');
            set(hy,'visible','off','color',color.ycline,'linestyle','--','LineWidth',2);
        else
            hy = zeros(0,1);
        end
        if minr < 0 && 0 < maxr
            [~,hr] = contour(Xx,Yy,DR,[0,0],'--');
            set(hr,'visible','off','color',color.xcline,'linestyle','--','LineWidth',2);
        else
            hr = zeros(0,1);
        end
        if minth < 0 && 0 < maxth
            [~,hth] = contour(Xx,Yy,DTheta,[0,0],'--');
            set(hth,'visible','off','color',color.ycline,'linestyle','--','LineWidth',2);
        else
            hth = zeros(0,1);
        end
        ...
        % Correction by DJ to update label string to match allowed strings.
        if strcmp(get(menull,'label'),'Hide nullclines.')
            set([hx;hy],'vis','on');
        elseif strcmp(get(mernull,'label'),'Hide polar nullclines.')
            set([hr;hth],'vis','on');
        end
        if ~isempty(notice) && notice~=0
            nstr = get(notice,'string');
    ...
    elseif strcmp(action,'hotcold')
        ...
        if (findstr(get(mehc,'label'),'inactive'))
            ...
            set(ppdisp,'WindowButtonDownFcn','pplane2017b_v2(''down'')');
    ...
    elseif strcmp(action,'down')
        ...
        pplane2017b_v2('solution',initpt,disph);
    ...
    elseif strcmp(action,'test case')
        ...
        for k = 1:10
            initpt = k*[-.2 .2];
            pplane2017b_v2('solution',initpt,ppdisp);
            initpt = k*[-.2 -.2];
            pplane2017b_v2('solution',initpt,ppdisp);
        end
        pplane2017b_v2('solution',[1,-1],ppdisp);
        dud = get(gcf,'user');
        nstr = get(dud.notice,'string');
        nstr(1:4) = nstr(2:5);
        nstr{5,1} = 'Ready.';
    ...
    elseif strcmp(action,'kcompute')
        ...
        % button on the pplane2017b_v2 Keyboard figure.

        compute = 1;
        kh = get(gcf,'user');
        ppdisp = kh.fig;
        if (isempty(ppdisp))
            pplane2017b_v2('confused');
        end
        dud = get(ppdisp,'user');
        ppdispa = dud.axes;
        aud = get(ppdispa,'user');
        ...
        for kk = 1:6
            ...
            if ~isempty(pn)
                if isempty(pv)
                    ...
                    xstr = pplane2017b_v2('paraeval',pn,pv,xstr);
                    ystr = pplane2017b_v2('paraeval',pn,pv,ystr);
                end
            end
        end
        xvalue = str2num(xstr);
    ...
    elseif strcmp(action,'kbd')
        ...
        % sets up the pplane2017b_v2 Keyboard figure which allows accurate input of
        ...
        ppkbd = findobj('name','pplane2017b_v2 Keyboard input');
        if  ~isempty(ppkbd)
            delete(ppkbd);
        end

        ppkbd = figure('name','pplane2017b_v2 Keyboard input',...
            'vis','off',...
            'tag','pplane2017b_v2');

        pplane2017b_v2('figdefault',ppkbd);
        ...
        kbd.comp = uicontrol('style','push',...
            'string','Compute','CallBack','pplane2017b_v2(''kcompute'')');
    ...
    elseif strcmp(action,'eqpt')
        ...
        z = pplane2017b_v2('newton',z0,dfcn);
        ...
        ppeqpt = findobj('name','pplane2017b_v2 Equilibrium point data');

        if (isempty(ppeqpt))
            ppeqpt = figure('vis','off','resize','on',...
                'name','pplane2017b_v2 Equilibrium point data',...
                'tag','pplane2017b_v2');
            pplane2017b_v2('figdefault',ppeqpt);
            ...
            ud.display = uicontrol('style','push',...
                ...
                'CallBack','pplane2017b_v2(''linear'')');
        else
            figure(ppeqpt);
            ud = get(ppeqpt,'user');
            set(ppeqpt,'vis','off');
    ...
    elseif strcmp(action,'zoomin')
        ...
        set(gcf,'WindowButtonDownFcn','pplane2017b_v2(''zoom'')',...
    ...
    elseif strcmp(action,'zoom')
        ...
        set(axh,'user',aud);
        set(disph,'user',dud);
        set(disph,'WindowButtonDownFcn','pplane2017b_v2(''down'')',...
            'WindowButtonUpFcn','');
        pplane2017b_v2('dirfield',disph);
        ppset = findobj('name','pplane2017b_v2 Setup');
        if isempty(ppset)
            pplane2017b_v2('confused');
        else
    ...
    elseif strcmp(action,'showbar')
        ...
        if strcmp(state,'figure')
            ...
            set(findobj(sbfig,'tooltipstr','Print'),...
                'clickedcallback','pplane2017b_v2(''print'');');
            switch name
                case 'pplane2017b_v2 Display'
                    set(findobj(sbfig,'tooltipstr','Zoom Out'),...
                        'clickedcallback',['pplane2017b_v2(''zoomback'');' fixtb]);
                    set(findobj(sbfig,'tooltipstr','Zoom In'),...
                        'clickedcallback',['pplane2017b_v2(''zoomin'');' fixtb]);
                    delete(findobj(sbfig,'tooltipstr','Rotate 3D'));

                case 'pplane2017b_v2 Linearization'
    ...
    elseif strcmp(action,'dall')
        ...
        dud = get(disph,'user');
        notice = dud.notice;
        kk = ishandle(dud.solhand);
        delete(dud.solhand(kk));
        dud.solhand = [];
        kk = ishandle(dud.ephand);
        delete(dud.ephand(kk));
        dud.ephand = [];
        dud.eqpts = [];
        kk = ishandle(dud.contours);
    ...
    elseif strcmp(action,'settings')
        ...
        % up the pplane2017b_v2 Settings window, which allows the user to interactively
        ...
        setfig = findobj('name','pplane2017b_v2 Settings');
        if ~isempty(setfig)
            delete(setfig);
        end

        setfig = figure('name','pplane2017b_v2 Settings',...
            ...
            'tag','pplane2017b_v2','vis','off');

        pplane2017b_v2('figdefault',setfig);
        ...
        chb = uicontrol('style','push',...
            ...
            'CallBack','pplane2017b_v2(''setchange'')');
    ...
    elseif strcmp(action,'setchange')
        ...
        % on the pplane2017b_v2 Settings window.
    ...
    elseif strcmp(action,'delete')
        ...
        trjh = dud.solhand;
        notice = dud.notice;
        if ~isempty(notice) && notice~=0   % Display window
            nstr = get(notice,'string');
            nstr(1:4) = nstr(2:5);
        ...
        set(disph,'user',dud);
        set(disph,'WindowButtonDownFcn',oldcall);
    ...
    elseif strcmp(action,'plotxy')  % Start a graph
        ...
        x=get(objh,'xdata');
        y=get(objh,'ydata');
        hh = pplane2017b_v2('plotxyfig',type,gcf);
        set(ppdisp,'WindowButtonDownFcn',oldcall);
        aud = get(hh(2),'user');
        ud = get(hh(1),'user');
        ud.data = [t;x;y];
        set(hh(1),'user',ud);
        pplane2017b_v2('plxy',aud.rad,hh(1));
    ...
    elseif strcmp(action,'plotxyfig')  % Build the graph figure.
        ...
        ppxy = figure('tag','pplane2017b_v2','visible','off');
        name = ['pplane2017b_v2 t-plot # ',int2str(ppxy.Number)];
        ud.color = dud.color;
        ud.type = 0;
        set(ppxy,'name',name,'user',ud);
        pplane2017b_v2('figdefault',ppxy);
        ...
        set(findobj(fmenu,'label','Print Pre&view...'),'pos',6);
        set(findobj(fmenu,'label','&Print...'),'pos',7);
        qmenu = uimenu(fmenu,'label','Quit pplane2017b_v2',...
            'CallBack','pplane2017b_v2(''quit'')',...
            'separator','on');
    ...
    elseif strcmp(action,'plxy')
        ...
        switch type
            case 1
                ...
                ylabel(xstring)
                set(r3dh,'vis','off')
                set(fig,'WindowButtonDownFcn','pplane2017b_v2(''plxybdf'')');
            case 2
    ...
    elseif strcmp(action,'crop')
        ...
        hh = pplane2017b_v2('plotxyfig',type,ppxy);
        ud = get(hh(1),'user');
        ud.data = [t;x;y];
        set(hh(1),'user',ud);
        aud = get(hh(2),'user');
    ...
    elseif strcmp(action,'plxybdf')
        ...
        dispa = gca;
        dispf = gcf;
        set(dispf,'windowbuttonmotionfcn','pplane2017b_v2(''plxybmf'')',...
            'windowbuttonupfcn','pplane2017b_v2(''plxybuf'')',...
            'inter','on');
        aud = get(dispa,'user');
        delete(aud.h);
        aud.h = [];
    ...
    elseif strcmp(action,'print')
        ...
        nstr = get(dud.notice,'string');
        nstr(1:4) = nstr(2:5);
        nstr{5} = 'Preparing to print the pplane2017b_v2 Display Window.  Please be patient.';
        set(dud.notice,'string',nstr);

        nstr(1:4) = nstr(2:5);
        nstr{5} = 'Printing the pplane2017b_v2 Display Window.';
        set(dud.notice,'string',nstr);
    ...
    elseif strcmp(action,'figdefault')
        ...
        fig = input1;
        set(fig,'CloseRequestFcn','pplane2017b_v2(''closefcn'')');
        ppset = findobj('name','pplane2017b_v2 Setup');
        sud = get(ppset,'user');
        ud = get(fig,'user');
        ud.ssize = sud.ssize;
        ...
        switch style
            case 'black'
                ...
                set(fig,'defaultaxescolor',[0.2 0.2 0.2])
    ...
    elseif strcmp(action,'zoominsq')
        ...
        set(gcf,'WindowButtonDownFcn','pplane2017b_v2(''zoomsqd'')',...
            'WindowButtonUpFcn','1;','inter','on');
        set(gca,'inter','on');
        dud = get(gcf,'user');
        nstr = get(dud.notice,'string');
    ...
    elseif strcmp(action,'zoomsqd')
        ...
        set(ppdispa,'user',aud);
        set(gcf,'windowbuttonmotionfcn','pplane2017b_v2(''zoomsqm'')',...
            'windowbuttonupfcn','pplane2017b_v2(''zoomsqu'')');
    ...
    elseif strcmp(action,'zoomsqu')
        ...
        set(disph,'user',dud);
        set(disph,'WindowButtonDownFcn','pplane2017b_v2(''down'')',...
            'WindowButtonMotionFcn','pplane2017b_v2(''cdisp'')',...
            'WindowButtonUpFcn','');
        pplane2017b_v2('dirfield',disph);
        ppset = findobj('name','pplane2017b_v2 Setup');
        if isempty(ppset)
    ...
    elseif strcmp(action,'level')
        ...
        Xname = dud.syst.xvar;
        Yname = dud.syst.yvar;
        Xname(abs(Xname)==92)=[];  % Remove \s if any.
        Yname(abs(Yname)==92)=[];
        pplevel = findobj('name','pplane2017b_v2 Level sets');
        if ~isempty(pplevel)
            delete(pplevel)
        end
        pplevel = figure('name','pplane2017b_v2 Level sets',...
            'vis','off',...
            'tag','pplane2017b_v2');

        pplane2017b_v2('figdefault',pplevel);
        ...
        lev.ch(3) = uicontrol('style','radio','horiz','center',...
            ...
            'value',0,...
            'vis','on',...
            'string','Let pplane2017b_v2 decide.');
        ...
        lev.proc = uicontrol('style','push',...
            'string','Proceed',...
            'CallBack','pplane2017b_v2(''levcomp'')');
        ...
        for kk = 1:4
            ...
            if ~isempty(pn)
                ...
                if isempty(pv)
                    ...
                    pv(abs(pv)==32)=[];
                    lfcn = pplane2017b_v2('paraeval',pn,pv,lfcn);
                end
            end
        end
        l = length(lfcn);
    ...
    elseif strcmp(action,'closefcn')
        ...
        fig = gcf;
        name = get(fig,'name');
        if strcmp(name,'pplane2017b_v2 Setup') || strcmp(name,'pplane2017b_v2 Display')
            quest = ['Closing this window will cause all pplane2017b_v2 ',...
                'windows to close, and pplane2017b_v2 will stop.  ',...
                'Do you want to quit pplane2017b_v2?'];
            butt = questdlg(quest,'Quit pplane2017b_v2?','Quit','Cancel','Quit');
            if strcmp(butt,'Quit')
                pplane2017b_v2('quit');
            end
        elseif strcmp(name,'pplane2017b_v2 Linearization')
            dud = get(fig,'user');
            fcn = dud.function;
            if (exist(fcn, 'file')==2), delete([fcn,'.m']);end
    ...
    elseif strcmp(action,'confused')
        ...
        tstring = 'pplane2017b_v2 is totally confused';
        qstring = {['You will have to restart pplane2017b_v2 from '...
            ...
            'What do you want to do?'};
        bstr1 = 'Quit and restart pplane2017b_v2.';
        bstr2 = 'Just quit pplane2017b_v2.';
        bstr3 = 'Do nothing.';
        answer = questdlg(qstring,tstring,bstr1,bstr2,bstr3,bstr1);
        if strcmp(answer,bstr1)
            delete(findobj('tag','pplane2017b_v2'));
            pplane2017b_v2;return
    ...
    while ~stop
        ...
        if ishghandle(gstop)
            ...
            if (steps > minNsteps)
                ...
                if (MMf<=sinkeps*absh)
                    z0 = yy(:,refine+1);
                    zz = pplane2017b_v2('newton',z0,dfcn);
                    zz = zz(:,1);
                    ud.zz = zz;
                    nz = norm((zz-z0)./DY);
                    if nz <= 0.01
...
function output = ppout(t,y,flag,varargin)
    ...
    % PPOUT  is the output function for pplane2017b_v2.

    %  Copywright (c)  John C. Polking, Rice University
    %  Last Modified: January 21, 2001

    output = 0;
    ppdisp = findobj(get(0,'child'),'flat','name','pplane2017b_v2 Display'); %gca;   %
    dud = get(ppdisp,'user');
    ppdispa = dud.axes;
    dfcn = dud.function;
    ud = get(ppdispa,'user');
